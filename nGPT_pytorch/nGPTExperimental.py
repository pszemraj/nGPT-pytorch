...
class Attention(Module):
    ...
    def forward(self, x, mask=None, value_residual=None):
...
class nGPT(Module):
    ...
    def forward(self, ids, mask=None, return_loss=False, return_breakdown=False):
        token_embed = self.token_embed.weight

        not_self = ~torch.eye(
            token_embed.shape[-2], device=ids.device, dtype=torch.bool
        )
        ...
        for attn, ff in self.layers:
            (tokens, values), ortho_loss = attn(
                tokens, mask=mask, value_residual=value_residual
            )
